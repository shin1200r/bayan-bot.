...
def handle_text(message):
    ...
    # ДОМА
        ...
        bot.send_photo(message.chat.id, "AgACAgIAAxkBAAICx2pBZtg2C69bIoRxD60hEb104z71AAISG2sblMsRSgNKPSkRUB0jAQADAgADeQADPAQ", 
                       caption="🏠 *Дом №1*\nЦена: 7000 ₸ в сутки.\nКонтакт: Наталия 8 777 939 09 67.\nInstagram: @bulbul.realtor", parse_mode="Markdown")
    elif "🏠 Дом №2" in txt:
        bot.send_message(message.chat.id, "🏠 *Дом №2*\nУютный дом. Детали уточняйте.", parse_mode="Markdown")
    elif "🏠 Дом №3" in txt:
        bot.send_message(message.chat.id, "📢 *Дом №3*\nЗдесь может быть ваше объявление! Перейдите в '📢 Реклама'.", parse_mode="Markdown")
    ...
    # ЛЕГЕНДЫ
        ...
        bot.send_message(message.chat.id, "Легенда про Жасыбая...", parse_mode="Markdown")
    ...
    # РЕКЛАМА
        ...
        bot.send_message(message.chat.id, "Команда не найдена.", reply_markup=get_main_markup())
